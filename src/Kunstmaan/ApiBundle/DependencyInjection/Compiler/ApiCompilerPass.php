...
class ApiCompilerPass implements CompilerPassInterface
{
    ...
    public function process(ContainerBuilder $container)
    {
        ...
        foreach ( $apiTypes['type'] as $class => $serviceId ) {
        ...
        foreach ( $apiTypes['resolver'] as $class => $serviceId ) {
            if ( false === $container->has($serviceId) ) {
                $definition = new Definition(ApiResolver::class);
                $definition->setArguments([
                    $class, new Reference('doctrine.orm.entity_manager')
                ]);
                $container->setDefinition($serviceId, $definition);
            }
        }
        ...
        if ($filesystem->exists($directory)) {
            ...
            foreach ($finder as $fileInfo) {
                ...
                if ( $reflectionClass->isInterface() === false && $annotation != null ) {
                    $classes['type'][$class] = sprintf('kunstmaan_api.type.%s', $annotation->getName());
                    $classes['resolver'][$class] = sprintf('kunstmaan_api.resolver.%s', $annotation->getName());
                }
            }
        }
